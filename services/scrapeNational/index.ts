...
import { DEFAULT_SCRAPING_CONFIG } from './constants';
import { 
  httpClient, 
  searchEngineScraper, 
...
import { sleep, generateSearchCombinations, cleanBusinessData, validateBusinessData, formatBusinessDataForDB } from './helpers';

dotenv.config();

// TODO: This should be moved to constants.ts in a future refactor
// For now, we'll create a placeholder to make the code work
const ALL_STATE_CONFIGS: StateConfig[] = [
  // This would normally be imported from the config module
  // Placeholder implementation to avoid import errors
  {
    state: 'California',
    tier: 1,
    businessTarget: 500,
    cities: ['Los Angeles', 'San Francisco', 'San Diego'],
    metropolitanAreas: ['Los Angeles Metro', 'San Francisco Bay Area'],
...
export class NationalBusinessScraper {
  ...
  async processState(config: StateConfig): Promise<StateResult> {
    ...
    try {
      ...
      const searchCombinations = generateSearchCombinations(config);
      ...
      try {
        ...
        for (const url of searchResults) {
          try {
            ...
            const cleanedData = cleanBusinessData(details);
    ...
    try {
      ...
      const validBusinesses = businesses.filter(business => {
        ...
        if (!validation.isValid) {
          console.warn(`Invalid business data: ${business.name} - ${validation.errors.join(', ')}`);
          return false;
        }
        return true;
      });
      ...
      for (let i = 0; i < validBusinesses.length; i += 10) {
        const batch = validBusinesses.slice(i, i + 10);
        const formattedBatch = batch.map(business => formatBusinessDataForDB(business));
        
        await db.insert(businesses).values(formattedBatch);
        
        console.log(`Saved batch ${Math.floor(i / 10) + 1}/${Math.ceil(validBusinesses.length / 10)}`);
        
        // Apply rate limiting
        if (i + 10 < validBusinesses.length) {
          await sleep(this.config.rateLimitDelay);
        }
      }
